...
async function login() {
  ...
  try {
    ...
    while (page.url() == `${site}/auth/login`) {
...
async function getSamples(page, url) {
  ...
  for (let i = 0; i < samples.length; i++) {
    res.push({ in: await samples[i].evaluate(node => node.getElementsByClassName('input')[0].getElementsByTagName('pre')[0].textContent),
              ans: await samples[i].evaluate(node => node.getElementsByClassName('output')[0].getElementsByTagName('pre')[0].textContent) });
  }
  return res;
}
...
async function getContest(browser, contest) {
  ...
  for (let i = 0; i < problems.length; i++) {
    ...
    const pid_global = (await problems[i].evaluate(node => node.getElementsByClassName('title color-default')[0].href)).match(/problem\/([A-Za-z]+[0-9]+)/)[1];
...
async function get(id, file, data) {
  ...
  try {
    ...
    if (id.match(/problem\/([A-Za-z]+[0-9]+)?.*contestId=([0-9]+)/)) {
      const res = id.match(/problem\/([A-Za-z]+[0-9]+)?.*contestId=([0-9]+)/);
      const contest = res[2], problem = res[1];
      await getProblem(page, contest, problem, file, data);
    } else if (id.match(/problem\/([A-Za-z]+[0-9]+)/)) {
      const res = id.match(/problem\/([A-Za-z]+[0-9]+)/);
      const problem = res[1];
      await getProblem(page, undefined, problem, file, data);
    } else if (id.match(/^([A-Za-z]+[0-9]+)\s+([0-9]+)$/)) {
      const res = id.match(/^([A-Za-z]+[0-9]+)\s+([0-9]+)$/);
      const contest = res[2], problem = res[1];
      await getProblem(page, contest, problem, file, data);
    } else if (id.match(/^([A-Za-z]+[0-9]+)$/)) {
      const res = id.match(/^([A-Za-z]+[0-9]+)$/);
      const problem = res[1];
      await getProblem(page, undefined, problem, file, data);
...
async function submitProblem(page, contest, problem, code) {
  ...
  for (let i = 0; i < buttons.length; i++) {
    if ((await buttons[i].evaluate(node => node.textContent)).includes('提交')) {
      buttons[i].evaluate(node => node.click());
...
async function submit(file, id) {
  ...
  try {
    ...
    let contest, problem;
    if (code.match(/^\/\/\s*Atiro\s*:\s*Luogu\s+([A-Za-z][0-9]+)\s+([0-9]*)/)) {
      const res = code.match(/^\/\/\s*Atiro\s*:\s*Luogu\s+([A-Za-z][0-9]+)\s+([0-9]*)/);
      contest = res[2], problem = res[1];
      code = code.replace(/^\/\/\s*Atiro\s*:\s*Luogu\s+([A-Za-z][0-9]+)\s+([0-9]*)\s*/, '');
    } else if (code.match(/^\/\/\s*Atiro\s*:\s*Luogu\s+([A-Za-z][0-9]+)/)) {
      const res = code.match(/^\/\/\s*Atiro\s*:\s*Luogu\s+([A-Za-z][0-9]+)/);
      problem = res[1];
      code = code.replace(/^\/\/\s*Atiro\s*:\s*Luogu\s+([A-Za-z][0-9]+)\s*/, '');
    }
    if (id) {
      if (id.match(/problem\/([A-Za-z]+[0-9]+)?.*contestId=([0-9]+)/)) {
        const res = id.match(/problem\/([A-Za-z]+[0-9]+)?.*contestId=([0-9]+)/);
