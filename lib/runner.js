...
function writeJudgInfo(content, color) {
...
module.exports = async (file, data, interactor, options) => {
  try {
    ...
    if (!interactor) {
      ...
      const grader = spawn(interactor, [`${data}.in`, `${data}.out`], { stdio: ['pipe', 'pipe', 'inherit'], timeout: timeLimit });

      answer.stdout.pipe(grader.stdin);
      ...
      writeJudgInfo(`Return Value: ${res[0].value}\nRunning Time: ${res[0].time.toFixed(0)}ms`, '#85929E');

      if (res[0].signal || res[1].signal) {
        console.log(chalk.blue('[Time Limit Exceeded]'));
        return 1;
      } else if (res[0].value) {
        console.log(chalk.magenta('[Runtime Error]'));
