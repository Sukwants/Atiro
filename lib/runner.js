const { spawnSync } = require('child_process');
const fs = require('fs');
const chalk = require('chalk');

module.exports = (file, data) => {
  try {
    const input = fs.existsSync(`./${data}.in`) ? fs.readFileSync(`./${data}.in`).toString() : '';

    const startTime = process.hrtime.bigint();
    const result = spawnSync(`./${file}.exe`, { input: input, stdio: ['pipe', 'pipe', 'inherit'], timeout: 5000 });
    const endTime = process.hrtime.bigint();
    const runTime = Number(endTime - startTime) / 1e6;

    fs.writeFileSync(`./${data}.out`, result.stdout);
    let writeJudgInfo = (lines) => {
      let maxl = 0;
      for (let i = 0; i < lines.length; i++) {
        maxl = maxl > lines[i].length ? maxl : lines[i].length;
        if (!i) console.log('「' + lines[i]);
        else if (i == lines.length - 1) {
          let res = '';
          for (let j = 0; j < maxl - lines[i].length; j++) res = res + ' ';
          console.log('  ' + lines[i] + res + '」');
        }
        else console.log('  ' + lines[i]);
      }
    }
    writeJudgInfo([`Return Value: ${result.status}`,
                   `Running Time: ${runTime.toFixed(0)}ms`]);

    if (result.error) {
      if (result.signal) {
        console.log(chalk.blue('[Time Limit Exceeded]'));
      } else {
        console.log(chalk.magenta('[Runtime Error]'));
      }
      process.exit(0);
    }
  } catch (error) {
    console.log(chalk.gray('[Mysterious Error]'));
    process.exit(0);
  }
}