...
module.exports = (file, data, options) => {
  try {
    ...
    const result = spawnSync(`./${file}.exe`, {
      input: input, stdio: ['pipe', 'pipe', 'inherit'],
      timeout: !isNaN(parseInt(options.time)) ? parseInt(options.time) : 
               !isNaN(parseInt(config.getConfigValue('options', 'time'))) ? parseInt(config.getConfigValue('options', 'time')) :
                5000 });
