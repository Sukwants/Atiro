...
module.exports = async (file, inf, ouf, interactor, timeLimit) => {
  try {
    if (!interactor) {                                                         // traditional runner
      ...
      const result = spawnSync(file, { input: input, stdio: ['pipe', 'pipe', 'inherit'], timeout: timeLimit, maxBuffer: 64 * 1024 * 1024 });
      const endTime = process.hrtime.bigint();
      const runTime = Number(endTime - startTime) / 1e6;

      const currentTime = moment();
      const kdl = (currentTime.hour() == 9 && currentTime.minute() == 22) || (currentTime.hour() == 22 && currentTime.minute() == 9);
      writeJudgInfo(`Return Value: ${result.status == 0 && kdl ? 'kdl' : result.status}\nRunning Time: ${runTime.toFixed(0)}ms`, '#708090');
      ...
      const startTime = process.hrtime.bigint();
      const answer = spawn(file, { stdio: ['pipe', 'pipe', 'inherit'], timeout: timeLimit, maxBuffer: 64 * 1024 * 1024 });
      const grader = spawn(interactor, [inf, ouf], { stdio: ['pipe', 'pipe', 'inherit'], maxBuffer: 64 * 1024 * 1024 });
      ...
      const currentTime = moment();
      const kdl = (currentTime.hour() == 9 && currentTime.minute() == 22) || (currentTime.hour() == 22 && currentTime.minute() == 9);
      writeJudgInfo(`Return Value: ${res[0].value == 0 && kdl ? 'kdl' : res[0].value}\nRunning Time: ${res[0].time.toFixed(0)}ms`, '#708090');
