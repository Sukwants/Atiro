...
const direPath = path.join(HOME, `.${require('../package.json').name}`);

module.exports = {
  data: (cate, defaults, message) => {
    const filePath = path.join(direPath, `${cate}.json`);
  ...
  reset: () => {
    try {
      fs.removeSync(direPath);
      console.log(chalk.green('[Success]'), 'Cleared data successfully.');
    } catch {
      console.log(chalk.red('[Error]'), 'Failed on clearing data.');
    }
  }
}