...
async function check(page) {
  ...
  if (!text.toLowerCase().includes('enter') && !text.toLowerCase().includes('войти')) return true;
  else return false;
}
...
async function login(options) {
  ...
  try {
    if (options.cookies != undefined) {
      ...
      if (text.toLowerCase().includes('enter') || text.toLowerCase().includes('войти')) {
        throw 1;
      }
...
async function submitProblem(page, contest, problem, code) {
  ...
  await page.waitForFunction(() => {
    return !document.getElementById('singlePageSubmitButton').disabled;
  }, { timeout: 5000 });
  ...
  if (!page.url().includes('/my')) {
    if (page.url().includes('/submit')) {
      const text = await page.evaluate(() => {
        const element = document.getElementsByClassName('error for__source');
        return element ? element[0].innerText : null;
      });
      console.log(chalk.red('[Error]'), text.trim().replace(/\.?$/, '.'));
      exit(1);
    }
    else throw 1;
  }
