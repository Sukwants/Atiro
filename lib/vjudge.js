...
async function getSamples(page, url) {
  ...
  for (let i = 0; i < samples.length; i++) {
    res.push({ in: await samples[i].evaluate(node => node.children[0].innerText),
              ans: await samples[i].evaluate(node => node.children[1].innerText) });
  }
  return res;
}
...
async function submitProblem(page, contest, problem, code) {
  ...
  await page.waitForFunction(() => {
    return !document.querySelector('div.modal-footer > #btn-submit').disabled;
  }, { timeout: 5000 });
  ...
  await page.waitForSelector('#solutionModalLabel > a:first-child', { timeout: 10000 });
  ...
  if (process.platform == 'win32') {
    ...
    exec(`open ${await page.$eval('#solutionModalLabel > a:first-child', node => node.href)}`);
  } else {
