...
module.exports = async (file, data, options) => {
  try {
    ...
    let checker = options.judg || config.getValue('option', 'judg');    // process checker file path
    ...
    let interactor = options.grad || config.getValue('option', 'grad'); // process interactor file path
    if (interactor) {
