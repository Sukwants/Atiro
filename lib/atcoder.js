...
async function getSamples(page, url) {
  ...
  for (let i = 0; i + 1 < samples.length; i++) {
    if (await samples[i].evaluate(node => node.parentElement.children[0].textContent.toLowerCase().includes('sample input'))
     && await samples[i + 1].evaluate(node => node.parentElement.children[0].textContent.toLowerCase().includes('sample output')))
...
async function submitProblem(page, contest, problem, code) {
  ...
  for (let i = 0, j = 0; i < lang_options.length; i++) {
    ...
    if (lang.toLowerCase().includes('c++') || lang.toLowerCase().includes('g++')) {
      ...
      if (await lang_options[i].evaluate(node => { return node.value == getLS('defaultLang'); })) {
        defaultLang = j;
      }
      j++;
    }
  ...
  await page.evaluate((code) => {
    $('.editor').data('editor').doc.setValue(code);
