...
async function getSamples(page, url) {
  ...
  for (let i = 0; i + 1 < samples.length; i++) {
    if (await samples[i].evaluate(node => { let text = node.parentElement.children[0].textContent.toLowerCase(); return text.includes('sample input') || text.includes('入力例'); })
     && await samples[i + 1].evaluate(node => { let text = node.parentElement.children[0].textContent.toLowerCase(); return text.includes('sample output') || text.includes('出力例'); }))
